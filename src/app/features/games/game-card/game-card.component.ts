...
export class GameCardComponent {
  ...
  readonly isCompleted = computed(() => {
    ...
    return game.status === 'completed' || (!game.status && Boolean(game.completionDate));
  });

  readonly cardClasses = computed(() => {
    const game = this.game();
    const classes = ['game-card'];
    if (game?.isBorrowedFrom) {
      classes.push('borrowed-from');
    }
    if (this.isCompleted()) {
      classes.push('completed');
    }
    if (game?.isBorrowedFrom && this.isCompleted()) {
      classes.push('has-both-badges');
    } else if (game?.isBorrowedFrom || this.isCompleted()) {
      classes.push('has-top-badge');
    }
    return classes.join(' ');
  });
